 ...
 * 
 * 生成データ数：35件
 * この数はページネーションのテストに適した数です。
 * （1ページ7件表示の場合、5ページ分のデータ）
 * 
 * 注意：
 * このシーダーはテスト用であり、本番環境での実行は推奨しません。
 */
class ContactSeeder extends Seeder
{
    ...
    public function run()
    {
        // ContactFactoryを使用して、一度に35件のテストデータを生成・保存
        // factory(35)で生成数を指定、create()で実際にデータベースに保存
        Contact::factory(35)->create();

        // 注意：この処理は数秒から数十秒かかる場合があります
        // 大量のデータを生成する場合は、chunk()やDBトランザクションの使用を検討してください
    }
}
